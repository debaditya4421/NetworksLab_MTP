    ...
    while(1) {
        ...
        for(int i=0; i<MAX_MTP_SOCKETS; i++) {
            if(FD_ISSET(shared_memory[i].udp_socket_id, &readfds)) {
                ...
                if(recv_msg[msg_len-1]=='2'){
                    // special msg ACK-n-2
                    recv_msg[msg_len-1] = '\0'; msg_len--;
                    recv_msg[msg_len-1] = '\0'; msg_len--;   
                    int ch = recv_msg[msg_len-1] - 48;

                    // sender thread will recv this special msg from receiver
                    shared_memory[i].send_max = ch;
                ...
                if(recv_msg[msg_len-1] == '1'){
                    ...
                    if(arr1[p2]==-1){
                        ...
                        int u3=0;
                        for (int w1 = 0; w1 < MAX_RECV_BUF_SZ; w1++)
                        {
                            if(strlen(shared_memory[i].recv_buffer[w1]) == 0){
                                u3++;
                            }
                        }

                        // shared_memory[0].send_max = u3;

                        struct sockaddr_in dest_addr = shared_memory[i].dest_addr;
                        char message[MAX_MESSAGE_SIZE];
                        set_null(message, MAX_MESSAGE_SIZE);
                        sprintf(message, "ACK-%d-%d-0", u3, seq_no);
                        int status = send_message(sockfd, &dest_addr, message);
                        if (status != -1) {
                            printf("Dup Ack sent @ %s\n" , message);
                            shared_memory[i].time_recv = time(NULL);
                        }
                        sleep(2);
                    }
                    else if(hh==0){
                    ...
                    for(int t=msg_len-1;t>=0;t--){
                        if(recv_msg[t]=='-'){
                            shared_memory[i].send_max = recv_msg[t-1]-48;
                            break;
                        }
                    }
                    // V(csmutex);
        ...
        for(int i=0; i<MAX_MTP_SOCKETS; i++){
            ...
            if(time(NULL)-shared_memory[i].time_recv > 18 && !(shared_memory[i].wr1==0 && shared_memory[i].wr2==0)){
                
                P(csmutex);
                printf("Ready to send special ack\n");
                struct sockaddr_in dest_addr = shared_memory[i].dest_addr;
                int u4=0;
                for (int w1 = 0; w1 < MAX_RECV_BUF_SZ; w1++){
                    if(strlen(shared_memory[i].recv_buffer[w1]) == 0){
                ...
                char message[MAX_MESSAGE_SIZE];
                set_null(message, MAX_MESSAGE_SIZE);
                sprintf(message, "ACK-%d-2", u4);
                int wrr = shared_memory[i].wr2 - shared_memory[i].wr1 + MAX_RECV_BUF_SZ;
                printf("u4  = %d , wr = %d\n",u4,wrr);
                socklen_t sockfd = shared_memory[i].udp_socket_id;
                int status = send_message(sockfd, &dest_addr, message);
                if (status != -1) {
                    printf("Special Ack sent @ %s\n" , message);
                    shared_memory[i].time_recv = time(NULL);
                }
                sleep(1);
                V(csmutex);
...
void *thread_S(void *arg) {
    ...
    while(1) {
        ...
        for(int i=0; i<MAX_MTP_SOCKETS; i++) {
            ...
            if (time(NULL) - shared_memory[i].timestamp >= 10 && shared_memory[i].swnd.size>0 ) {
                P(csmutex);
                ...
                for(int j=pos; j<pos + shared_memory[i].swnd.size; j++) {
                    ...
                    char message[MAX_MESSAGE_SIZE];
                    set_null(message,MAX_MESSAGE_SIZE);
                    strcpy(message, shared_memory[i].send_buffer[j%MAX_SEND_BUF_SZ]);

                    int ff = shared_memory[i].swnd.sequence_numbers[0];
                    if(ff==-2) break;
                    int val = (ff+j-pos)%16 ;
                    sprintf(message, "%s-%d-1", message, val);
                    int status = send_message(sockfd, &dest_addr, message);
                    printf("Seq no %d ;Msg: %s\n",val,message);
                    if(status!=-1){
                        // P(csmutex);
                        shared_memory[i].timestamp = time(NULL);
                        printf("timeout msg sent\n");
                        // V(csmutex);
                    }
                }
                V(csmutex);
...
void *garbage_collector(void *arg) {
    ...
    while(1){
        ...
        for(int i=0; i<MAX_MTP_SOCKETS; i++) {
            P(csmutex);
            if (kill(shared_memory[i].process_id,NULL)==-1) {
                // clear up the process
                printf("Cleared up the MTP socket : %d PID = %d\n",i,shared_memory[i].process_id);
                shared_memory[i].process_id = -1;
...
int main() {
    ...
    if (pthread_create(&garbage_collector_id, NULL, garbage_collector, shared_memory) != 0) {
        printf("Error creating garbage collector\n");
        exit(EXIT_FAILURE);
    }
    ...
    while(1){
        ...
        shared_memory[i1].wr1 = 0;
        shared_memory[i1].wr2 = 0;
        shared_memory[i1].time_recv = time(NULL);
